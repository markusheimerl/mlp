...
MLP* init_mlp(int input_dim, int hidden_dim, int output_dim, int batch_size) {
    ...
    mlp->W2 = (float*)malloc(output_dim * hidden_dim * hidden_dim * sizeof(float));
    mlp->W3 = (float*)malloc(output_dim * input_dim * sizeof(float));
    mlp->W1_grad = (float*)malloc(hidden_dim * input_dim * sizeof(float));
    mlp->W2_grad = (float*)malloc(output_dim * hidden_dim * hidden_dim * sizeof(float));
    ...
    mlp->W2_m = (float*)calloc(output_dim * hidden_dim * hidden_dim, sizeof(float));
    mlp->W2_v = (float*)calloc(output_dim * hidden_dim * hidden_dim, sizeof(float));
    ...
    mlp->outer_product = (float*)malloc(batch_size * hidden_dim * hidden_dim * sizeof(float));
...
void free_mlp(MLP* mlp) {
    ...
    free(mlp->layer1_output); free(mlp->layer2_output);
    free(mlp->error_output); free(mlp->error_hidden);
    free(mlp->outer_product);
    free(mlp);
}
...
void forward_pass_mlp(MLP* mlp, float* X) {
    ...
    for (int b = 0; b < mlp->batch_size; b++) {
        cblas_sger(CblasRowMajor, mlp->hidden_dim, mlp->hidden_dim,
                   ...
                   &mlp->layer1_output[b * mlp->hidden_dim], 1,
...
void zero_gradients_mlp(MLP* mlp) {
    ...
    memset(mlp->W2_grad, 0, mlp->output_dim * mlp->hidden_dim * mlp->hidden_dim * sizeof(float));
...
void backward_pass_mlp(MLP* mlp, float* X) {
    ...
    for (int b = 0; b < mlp->batch_size; b++) {
        cblas_sgemv(CblasRowMajor, CblasNoTrans, mlp->hidden_dim, mlp->hidden_dim,
                    ...
                    &mlp->layer1_output[b * mlp->hidden_dim], 1,
                    0.0f, &mlp->error_hidden[b * mlp->hidden_dim], 1);
        
        cblas_sgemv(CblasRowMajor, CblasTrans, mlp->hidden_dim, mlp->hidden_dim,
                    1.0f, &mlp->outer_product[b * mlp->hidden_dim * mlp->hidden_dim], mlp->hidden_dim,
                    &mlp->layer1_output[b * mlp->hidden_dim], 1,
                    1.0f, &mlp->error_hidden[b * mlp->hidden_dim], 1);
    }
...
void update_weights_mlp(MLP* mlp, float learning_rate) {
    ...
    // Update W2 weights
    for (int i = 0; i < mlp->output_dim * mlp->hidden_dim * mlp->hidden_dim; i++) {
...
void save_mlp(MLP* mlp, const char* filename) {
    ...
    fwrite(mlp->W2, sizeof(float), mlp->output_dim * mlp->hidden_dim * mlp->hidden_dim, file);
    ...
    fwrite(mlp->W2_m, sizeof(float), mlp->output_dim * mlp->hidden_dim * mlp->hidden_dim, file);
    fwrite(mlp->W2_v, sizeof(float), mlp->output_dim * mlp->hidden_dim * mlp->hidden_dim, file);
...
MLP* load_mlp(const char* filename, int custom_batch_size) {
    ...
    fread(mlp->W2, sizeof(float), output_dim * hidden_dim * hidden_dim, file);
    ...
    fread(mlp->W2_m, sizeof(float), output_dim * hidden_dim * hidden_dim, file);
    fread(mlp->W2_v, sizeof(float), output_dim * hidden_dim * hidden_dim, file);
