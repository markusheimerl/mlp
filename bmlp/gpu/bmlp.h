...
typedef struct {
    ...
    float* d_W2;     // output_dim x (hidden_dim * hidden_dim)
    float* d_W3;     // output_dim x input_dim
    float* d_W1_grad; // hidden_dim x input_dim
    float* d_W2_grad; // output_dim x (hidden_dim * hidden_dim)
    ...
    float* d_outer_product;   // batch_size x (hidden_dim * hidden_dim)
    float* d_outer_grad;      // batch_size x (hidden_dim * hidden_dim)
...
__global__ void adamw_update_kernel_bmlp(float* weight, float* grad, float* m, float* v, float beta1, float beta2, float epsilon, float learning_rate, float weight_decay, float alpha_t, int size, int batch_size);
