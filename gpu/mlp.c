...
MLP* init_mlp(int input_dim, int hidden_dim, int output_dim, int batch_size, cublasLtHandle_t cublaslt_handle) {
    ...
    CHECK_CUDA(cudaMalloc(&mlp->d_W1_m, w1_size * sizeof(__half)));
    CHECK_CUDA(cudaMalloc(&mlp->d_W1_v, w1_size * sizeof(__half)));
    CHECK_CUDA(cudaMalloc(&mlp->d_W2_m, w2_size * sizeof(__half)));
    CHECK_CUDA(cudaMalloc(&mlp->d_W2_v, w2_size * sizeof(__half)));
    ...
    CHECK_CUDA(cudaMemset(mlp->d_W1_m, 0, w1_size * sizeof(__half)));
    CHECK_CUDA(cudaMemset(mlp->d_W1_v, 0, w1_size * sizeof(__half)));
    CHECK_CUDA(cudaMemset(mlp->d_W2_m, 0, w2_size * sizeof(__half)));
    CHECK_CUDA(cudaMemset(mlp->d_W2_v, 0, w2_size * sizeof(__half)));
...
__global__ static void adamw_update_kernel_mlp(float* weight, float* grad, __half* m, __half* v,
...
void serialize_mlp(MLP* mlp, FILE* file) {
    ...
    float* h_W1 = (float*)malloc(w1_size * sizeof(float));
    float* h_W2 = (float*)malloc(w2_size * sizeof(float));
    __half* h_W1_m = (__half*)malloc(w1_size * sizeof(__half));
    __half* h_W1_v = (__half*)malloc(w1_size * sizeof(__half));
    __half* h_W2_m = (__half*)malloc(w2_size * sizeof(__half));
    __half* h_W2_v = (__half*)malloc(w2_size * sizeof(__half));
    ...
    CHECK_CUDA(cudaMemcpy(h_W1_m, mlp->d_W1_m, w1_size * sizeof(__half), cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(h_W1_v, mlp->d_W1_v, w1_size * sizeof(__half), cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(h_W2_m, mlp->d_W2_m, w2_size * sizeof(__half), cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(h_W2_v, mlp->d_W2_v, w2_size * sizeof(__half), cudaMemcpyDeviceToHost));
    
    // Write to file
    fwrite(h_W1, sizeof(float), w1_size, file);
    fwrite(h_W2, sizeof(float), w2_size, file);
    fwrite(&mlp->t, sizeof(int), 1, file);
    fwrite(h_W1_m, sizeof(__half), w1_size, file);
    fwrite(h_W1_v, sizeof(__half), w1_size, file);
    fwrite(h_W2_m, sizeof(__half), w2_size, file);
    fwrite(h_W2_v, sizeof(__half), w2_size, file);
...
MLP* deserialize_mlp(FILE* file, int batch_size, cublasLtHandle_t cublaslt_handle) {
    ...
    fread(h_W1_m, sizeof(__half), w1_size, file);
    fread(h_W1_v, sizeof(__half), w1_size, file);
    fread(h_W2_m, sizeof(__half), w2_size, file);
    fread(h_W2_v, sizeof(__half), w2_size, file);
    
    // Copy to device
    CHECK_CUDA(cudaMemcpy(mlp->d_W1, h_W1, w1_size * sizeof(float), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(mlp->d_W2, h_W2, w2_size * sizeof(float), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(mlp->d_W1_m, h_W1_m, w1_size * sizeof(__half), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(mlp->d_W1_v, h_W1_v, w1_size * sizeof(__half), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(mlp->d_W2_m, h_W2_m, w2_size * sizeof(__half), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(mlp->d_W2_v, h_W2_v, w2_size * sizeof(__half), cudaMemcpyHostToDevice));
