...
Tensor* tensor_multihead_attention(Tensor* Q, Tensor* K, Tensor* V, int num_heads) {
    ...
    if (d_model % num_heads != 0 || K->dims[2] != d_model || V->dims[2] != d_model || seq_len_k != V->dims[1] || batch_size != K->dims[0] || batch_size != V->dims[0]) return NULL;
...
int main() {
    {
        int batch_size = 1, seq_len = 2, d_model = 4, num_heads = 2, qkv_dims[] = {batch_size, seq_len, d_model};
        float q_data[] = {1,1,0,0, 0,0,1,1}, k_data[] = {1,1,0,0, 0,0,1,1}, v_data[] = {1,1,2,2, 3,3,4,4};
        Tensor *Q = tensor_new(3, qkv_dims, q_data, 1), *K = tensor_new(3, qkv_dims, k_data, 1), *V = tensor_new(3, qkv_dims, v_data, 1);
        ...
        for (int i = 0; i < seq_len; i++) {
            printf("Seq %d:", i);
            for (int j = 0; j < d_model; j++) printf(" %6.3f", K->grad[i * d_model + j]);
            printf("\n");
        }
    }
    {
        int batch_size = 1, seq_len = 3, d_model = 4, num_heads = 2, qkv_dims[] = {batch_size, seq_len, d_model};
        float q_data[12], k_data[12], v_data[12];
        for (int i = 0; i < seq_len * d_model; i++) { q_data[i] = k_data[i] = 1.0f; v_data[i] = (i/d_model) + 1.0f; }
        Tensor *Q = tensor_new(3, qkv_dims, q_data, 1), *K = tensor_new(3, qkv_dims, k_data, 1), *V = tensor_new(3, qkv_dims, v_data, 1);
        Tensor* output = tensor_multihead_attention(Q, K, V, num_heads);
        printf("\nTest 2: Uniform Attention Pattern\nOutput (should show uniform mixing):\n");
        for (int i = 0; i < seq_len; i++) {
            printf("Seq %d: ", i);
            for (int j = 0; j < d_model; j++) printf("%6.3f ", output->data[i * d_model + j]);
            printf("\n");
    ...
    {
        int batch_size = 1, seq_len = 2, d_model = 4, num_heads = 2, dims[] = {batch_size, seq_len, d_model};
        float q_data[] = {1,1,0,0, 0,0,1,1}, k_data[] = {1,1,0,0, 0,0,1,1}, v_data[] = {1,1,2,2, 3,3,4,4};
        Tensor *Q = tensor_new(3, dims, q_data, 1), *K = tensor_new(3, dims, k_data, 1), *V = tensor_new(3, dims, v_data, 1);
        Tensor* output = tensor_multihead_attention(Q, K, V, num_heads);
        for (int i = 0; i < output->size; i++) output->grad[i] = 1.0f;
        backward();
    ...
    {
        int batch_size = 2, seq_len = 3, d_model = 4, qkv_dims[] = {batch_size, seq_len, d_model};
        float data[24] = {0};
        Tensor *Q = tensor_new(3, qkv_dims, data, 1), *K = tensor_new(3, qkv_dims, data, 1), *V = tensor_new(3, qkv_dims, data, 1);
        printf("\nTest 4: Error Cases\nInvalid num_heads (should be NULL): %s\n", tensor_multihead_attention(Q, K, V, 3) == NULL ? "PASS" : "FAIL");
