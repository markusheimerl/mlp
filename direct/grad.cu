...
int main() {
    ...
    for(int i = 0; i < 10; i++) {
        cuda_forward(net, data->X[i]);
        cudaMemcpy(output, net->layers[net->n_layers-1].x, 4 * sizeof(double), cudaMemcpyDeviceToHost);
    ...
    // Save network and data with proper time handling
    time_t current_time = time(NULL);
    struct tm* timeinfo = localtime(&current_time);
    char net_fname[64], data_fname[64];
    
    strftime(net_fname, sizeof(net_fname), "%Y%m%d_%H%M%S_net.bin", timeinfo);
    cuda_save_net(net_fname, net);
    cuda_free_net(net);
    
    strftime(data_fname, sizeof(data_fname), "%Y%m%d_%H%M%S_data.csv", timeinfo);
    ...
    for(int i = 0; i < 10; i++) {
        ...
        cudaMemcpy(output, loaded_net->layers[loaded_net->n_layers-1].x, 4 * sizeof(double), cudaMemcpyDeviceToHost);
