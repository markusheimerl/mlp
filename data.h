...
char* get_timestamp_filename(const char* prefix) {
    ...
    strftime(filename, 100, "%Y%m%d_%H%M%S_", localtime(&now));
...
double complex_function(double x1, double x2, double x3, double x4, int output) {
    switch(output) {
        case 0: return sin(x1 * 2.0) * cos(x2 * 1.5) + pow(x3, 2) * x4 + 
                      exp(-pow(x1 - x2, 2)) + 0.5 * sin(x1 * x2 * 3.14159);
        case 1: return tanh(x1 + x2) * sin(x3 * 2.0) + log(fabs(x4) + 1) * 
                      cos(x1) + 0.3 * pow(x2 - x3, 3);
        case 2: return exp(-pow(x1 - 0.5, 2)) * sin(x2 * 3.0) + 
                      pow(cos(x3), 2) * x4 + 0.2 * sinh(x1 * x2);
        default: return 0.0;
    }
}
...
Data* synth(int n, int fx, int fy, double noise) {
    ...
    for(int i = 0; i < n; i++) {
        ...
        for(int j = 0; j < fx; j++) 
            d->X[i][j] = (double)rand()/RAND_MAX * 4 - 2;
        
        for(int j = 0; j < fy; j++) {
            d->y[i][j] = complex_function(d->X[i][0], d->X[i][1], d->X[i][2], d->X[i][3], j) +
                         0.2 * sin(d->X[i][0] * 10.0) + ((double)rand()/RAND_MAX - 0.5) * noise;
        }
    }
    return d;
}
